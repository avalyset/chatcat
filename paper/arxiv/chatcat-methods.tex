% chatcat-methods.tex — arXiv preprint (cs.LG only)
% Source: paper/sections/*-en.md + paper/tables/*-en.md (on origin/main).
...
\subsection{The gap: validating thresholds against window noise}
\label{sec:1.2}
...
\subsection{Contribution: the criterion-validity gate}
\label{sec:1.3}
...
The fragility of RL results is well documented. \citet{henderson2018deep} showed
that published reinforcement-learning numbers often fail to hold up: variance
across seeds, implementations, and hyperparameters is large enough that a
reported result can be an artefact of the particular run rather than a property
of the algorithm. Our gate addresses a specific sub-case of that fragility. The
problem \citet{henderson2018deep} document sits at the level of the result ---
the number moves when you rerun it. The gate moves the check one level up, to
the criterion: not whether the result varies, but whether the threshold the
result is measured against is separable from the noise in its own window. A
threshold sitting inside that noise produces unstable verdicts no amount of
careful result-reporting can rescue, because the instability is in the measuring
instrument, not the measurement.

The closest neighbour is \citet{agarwal2021deep}. rliable gives reliable
evaluation on few seeds --- stratified bootstrap confidence intervals,
performance profiles, interquartile means in place of point estimates --- and
is the direct methodological precursor to what we do. The difference is worth
stating plainly, because the two are easy to conflate. rliable quantifies the
uncertainty in a result: given the seeds you have, how wide is the interval
around the number. The gate asks a prior question --- whether the criterion is
applicable at all before the result is computed. The two are complementary and
sit at different stages of the pipeline: rliable is the right tool once you are
measuring; the gate is a check on whether the thing you are measuring against
holds up in the window where you apply it. We do not replace reliable
result-reporting; we add a step in front of it.

\citet{colas2018howmany} is the most direct thematic precursor. ``How Many
Random Seeds?'' and the statistical-comparison guide \citep{colas2019hitchhiker}
took up exactly seed variance and statistical separability in RL evaluation: how
many seeds are needed before a difference between two methods is real rather
than noise. The gate asks that same question one level deeper.
\citet{colas2018howmany} ask whether there are enough seeds to separate method
$A$ from method $B$; the gate asks whether the threshold doing the separating is
itself separable from the noise in the window it is applied to. The seed-wander
result in Section~\ref{sec:3.5} --- where the gate verdict itself moves with the
draw --- is a concrete instance of the concern \citet{colas2018howmany} raised,
surfacing one level up from the comparison they studied.

\citet{gundersen2018state} measured the state of reproducibility in AI
empirically: degree of documentation, what fraction of reported work can in fact
be reproduced. That work diagnoses the field; it measures whether AI research is
reproducible. The gate contributes on the other side of that diagnosis --- not a
measurement of the gap, but a piece of infrastructure that lets pre-registration
deliver reproducibility rather than merely reveal its absence. Where
\citet{gundersen2018state} establish that reproducibility is low, the gate is
one mechanism for raising it: a pre-registered check that keeps a threshold from
being applied where it cannot carry a stable verdict.
...
\begin{tabular}{@{}l p{2.0cm} l p{1.4cm} p{1.5cm} c p{2.8cm}@{}}
\toprule
...
\begin{tabular}{@{}r r r c c p{4.3cm}@{}}
\toprule
...
On the seeds that reproduce climb-then-slide, the SIG-EXPLORATION signature holds (Fig.~\ref{fig:3}). The variance in the policy does not collapse during peak, and the critic is converged low. The slide is the optimisation side of a wide-variance policy, not a result of variance collapse. The pattern is the same on all reproducing seeds, across both measurement passes.
...
\small $T = 0.0922$ (locked in the original pre-registration, ADR 0010). The gate was triggered in opposite directions on the two batches despite identical training configuration --- evidence that the gate is not a formality and that same-config noise scale is itself substantial.
\end{table}

\subsection{The gate verdict is itself seed-variable}
\label{sec:3.5}

The FAIL was a slice outcome. The N=15 escalation computed its FAIL --- $T/\sigma_\mathrm{diff} = 1.80$ --- on the ten new seeds $\{11 \ldots 20\}$. But the verdict depends on which seeds are drawn: the full set $\{6 \ldots 20\}$ gives $T/\sigma_\mathrm{diff} = 2.1459$ --- a bare PASS. Same corpus, different slice, opposite verdict.

The verdict-stability resampling pre-registered this as a question and measured it. Resampling across the fifteen seeds --- exhaustive choose-k for $k=5$ and $k=10$, the full set for $k=15$ --- yields the distribution of the gate verdict itself (Fig.~\ref{fig:4}). At $k=5$ the PASS rate is $0.5734$, a coin flip; at $k=10$ it is $0.7063$. The distribution sits centred just above the threshold --- $k=5$ median ratio $2.146$ --- yet $43\%$ of draws fall below $2.0$. The verdict landed in the pre-registered mid-band $[0.20, 0.80)$: the gate verdict is intrinsically seed-variable.

The mechanism is not the one Section~\ref{sec:3.4} implied --- and the resampling corrects it. A Brown-Forsythe test on $\{6 \ldots 10\}$ against $\{11 \ldots 20\}$ gives $W = 0.000159$, $p = 0.99$: there is no detectable scale difference between the batches. The apparent $\sim 50\%$ gap in median noise ($0.0239$ against $0.0362$) is not a noisier second batch. It is seed-wander across the noise floor $\sigma^* = T/(2\sqrt{2}) = 0.0326$. The population straddles the floor, so the median's position --- and with it the verdict --- depends on the draw, not on the batch. Where Section~\ref{sec:3.4} read the two seed sets as genuinely different, they are one set straddling a common floor.

This is the endpoint the resampling named in advance, now realised --- the earned, quantified form of ``measurability is seed-variable'': a real endpoint, not a call for more compute.

\begin{figure}[!t]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig4_gate_ratio_distribution.pdf}
\caption{Distribution of the gate-verdict ratio $T/\sigma_\mathrm{diff}$ across the fifteen seeds ($k=5$ resampling, all $3003$ draws), with the threshold at $2.0$ marked. The distribution straddles the threshold: PASS rate $0.5734$, median ratio $2.146$, with $43\%$ of draws below $2.0$. A Brown-Forsythe test ($p = 0.99$) shows that the apparent difference in median noise between seed batches is not a real scale difference --- the verdict depends on which seeds are drawn, not on the batch.}
\label{fig:4}
\end{figure}
...
Without the gate, the criterion-validity reanalysis's $M' = 3/5$ would have been reported as a borderline finding about the agent, and the N=15 escalation's new seeds would have produced an $M''$ figure that looked like data. The gate showed that both would in fact have been findings about the measurement window, not the agent, and refused to produce a clean number where the number would have been an artefact. Within RL evaluation, it generalises this far and no further: any threshold-based reproducibility assessment where the threshold is anchored in one regime and applied in another is vulnerable to this failure, and the gate is a cheap check against it. It is a check against one failure class, not a solution to RL evaluation.
...
Resolution (main run $\{6 \ldots 10\}$, F3 triggered) & 2026-06-05 & $M = 2/5$ against locked criterion; climb-window confounder documented \\
\multicolumn{3}{l}{\textbf{ADR 0011 --- criterion-validity gate introduced}} \\
Stub (gate pre-registered)                          & 2026-06-05 & Revised \code{ep\_init} window, gate formula, three-way $M'$ outcome \\
Resolution (reanalysis, gate PASS, $M' = 3/5$)      & 2026-06-05 & $T/\sigma_\mathrm{diff} = 2.7257$ PASS; three seeds flipped CTS status \\
\multicolumn{3}{l}{\textbf{ADR 0012 --- escalation to N=15}} \\
Stub (N=15 + three-way success + middle-band discipline) & 2026-06-05 & Seeds $\{11 \ldots 20\}$, middle band as endpoint \\
Resolution (gate FAIL, $M''$ not tallied)           & 2026-06-06 & $T/\sigma_\mathrm{diff} = 1.8027$ FAIL; $\sim 50\%$ noise-scale discrepancy \\
\multicolumn{3}{l}{\textbf{ADR 0013 --- verdict-stability resampling} \quad (substrate: OSF DOI \href{https://doi.org/10.17605/OSF.IO/SCX59}{10.17605/OSF.IO/SCX59})} \\
...
Resolution (mid-band: verdict seed-variable)        & 2026-06-12 & $k=5$ PASS-rate $0.5734 \in [0.20, 0.80)$; Brown-Forsythe $p = 0.99$ --- seed-wander, not a noisier batch \\
\multicolumn{3}{l}{\textbf{Supporting work}} \\
Instrumentation (\code{actor\_logstd} logging + checkpoint-on-best) & 2026-06-04 & \code{metrics.jsonl} schema extended for SIG-EXPLORATION reading \\
Figures + plotting script                           & 2026-06-06 & Fig.~1--4 + Table~1--2 reproducible from \code{metrics.jsonl} \\
Source-attribution correction (4 verified refs)     & 2026-06-06 & The four Section~\ref{sec:1.1} citations locked against verified originals \\
\bottomrule
\end{tabular}
\end{table}

\noindent The full reproducible substrate for the verdict-stability analysis (ADR 0013) --- the 15-seed frozen summary, the runnable gate and CTS tally, the freezing script, and the exhaustive $k=5$ gate-verdict-ratio distribution (all $3003$ draws) --- is archived as an immutable, timestamped OSF Registration: DOI \href{https://doi.org/10.17605/OSF.IO/SCX59}{10.17605/OSF.IO/SCX59}. This is the durable, citable anchor for the analysis, independent of any repository commit.

\subsection{Reproducibility}
\label{app:A.2}

{\sloppy
All training runs live as \code{metrics.jsonl} + \code{agent.pt} + \code{best\_so\_far.pt} under \code{\textasciitilde/chatcat-rl-runs/} (persistent path, documented in ADR 0010 Step 1.5). The plotting script \code{paper/plot\_methods\_figures.py} regenerates Fig.~1--3 and Tables~1--2 from the same \code{metrics.jsonl} files; no new training is required. Fig.~4 is regenerated from the committed artefact \code{paper/analysis/gate\_ratio\_distribution\_k5.csv} --- the exhaustive $k=5$ gate-verdict distribution produced by \code{paper/analysis/gate\_distribution.py} from the frozen \code{seed\_summary\_frozen.csv} --- so the verdict-stability figure is reproducible from committed substrate, not from a transient run. Vector export (PDF) for submission is produced by the same script.\par}
